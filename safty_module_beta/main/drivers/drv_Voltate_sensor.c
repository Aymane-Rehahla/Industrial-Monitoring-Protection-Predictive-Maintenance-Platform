/**
 * @file drv_voltage_sensor.c
 * @brief ZMPT101B voltage sensor implementation
 * @version 1.0.0
 * 
 * @safety HIGH
 * 
 * Rule 2.10: Rate-of-change checks for impossible jumps
 * Rule 4.3: All data timestamped
 */

#include "drv_voltage_sensor.h"
#include "app_config.h"
#include "time_utils.h"
#include "esp_log.h"

#include <string.h>
#include <math.h>

static const char *TAG = "DRV_VOLT";

/* ── Module state ────────────────────────────────────────────────────── */
static ads1115_handle_t *s_ads = NULL;
static voltage_calibration_t s_cal[3];
static int32_t s_last_mv[3] = {0, 0, 0};
static uint32_t s_last_time[3] = {0, 0, 0};
static bool s_initialized = false;

/* ── Constants ───────────────────────────────────────────────────────── */
#define MAX_RATE_MV_PER_SEC  10000   /* Max plausible rate of change */
#define ZMPT_CENTER_MV       2500    /* Center voltage at 0V input */

/* ── Default calibration ─────────────────────────────────────────────── */
static void set_default_calibration(voltage_calibration_t *cal)
{
    cal->scale_factor     = 0.311f;     /* Adjust based on actual ZMPT */
    cal->offset_mv        = 0;
    cal->divider_ratio    = VDIV_MULTIPLY;
    cal->calibration_date = 0;
    cal->is_calibrated    = false;
}

/* ── Public: Init ────────────────────────────────────────────────────── */
error_code_t voltage_sensor_init(ads1115_handle_t *ads_handle)
{
    if (ads_handle == NULL) { return ERR_NULL_POINTER; }
    
    s_ads = ads_handle;
    
    /* Initialize calibration to defaults */
    for (int i = 0; i < 3; i++) {
        set_default_calibration(&s_cal[i]);
        s_last_mv[i] = 0;
        s_last_time[i] = 0;
    }
    
    s_initialized = true;
    ESP_LOGI(TAG, "Voltage sensor initialized");
    return ERR_OK;
}

/* ── Public: Read single phase ───────────────────────────────────────── */
error_code_t voltage_sensor_read(uint8_t phase, voltage_reading_t *reading)
{
    /* Rule 2.1: NULL checks */
    if (reading == NULL) { return ERR_NULL_POINTER; }
    if (phase > 2) { return ERR_INVALID_PARAMETER; }
    if (!s_initialized || s_ads == NULL) { return ERR_NOT_INITIALIZED; }
    
    /* Initialize output */
    memset(reading, 0, sizeof(*reading));
    reading->channel      = phase;
    reading->timestamp_ms = get_time_ms();
    reading->quality      = QUALITY_INVALID;
    
    /* Read from ADS1115 */
    ads_reading_t adc;
    error_code_t err = ads1115_read_channel(s_ads, phase, &adc);
    
    if (err != ERR_OK) {
        ESP_LOGE(TAG, "Phase %d read failed: %d", phase, err);
        return err;
    }
    
    reading->raw_mv = adc.millivolts;
    
    /* Compensate for voltage divider */
    reading->compensated_mv = (int32_t)(adc.millivolts * s_cal[phase].divider_ratio);
    
    /* Apply calibration offset */
    int32_t corrected = reading->compensated_mv - s_cal[phase].offset_mv;
    
    /* Calculate voltage (simplified - for AC, would need RMS sampling) */
    /* For now: distance from center × scale factor */
    int32_t deviation = corrected - ZMPT_CENTER_MV;
    reading->voltage_rms_v = fabsf((float)deviation * s_cal[phase].scale_factor);
    
    /* Rule 2.10: Rate of change check */
    uint32_t now = get_time_ms();
    if (s_last_time[phase] > 0) {
        uint32_t dt = now - s_last_time[phase];
        if (dt > 0) {
            int32_t delta = abs(reading->compensated_mv - s_last_mv[phase]);
            int32_t rate = (delta * 1000) / (int32_t)dt;
            
            if (rate > MAX_RATE_MV_PER_SEC) {
                ESP_LOGW(TAG, "Phase %d rate violation: %ld mV/s", phase, (long)rate);
                reading->quality = QUALITY_DEGRADED;
            }
        }
    }
    
    s_last_mv[phase] = reading->compensated_mv;
    s_last_time[phase] = now;
    
    reading->is_valid = true;
    if (reading->quality == QUALITY_INVALID) {
        reading->quality = QUALITY_GOOD;
    }
    
    return ERR_OK;
}

/* ── Public: Read all phases ─────────────────────────────────────────── */
error_code_t voltage_sensor_read_all(voltage_reading_t readings[3])
{
    if (readings == NULL) { return ERR_NULL_POINTER; }
    
    error_code_t first_err = ERR_OK;
    
    for (uint8_t p = 0; p < 3; p++) {
        error_code_t err = voltage_sensor_read(p, &readings[p]);
        if (err != ERR_OK && first_err == ERR_OK) {
            first_err = err;
        }
    }
    
    return first_err;
}

/* ── Public: Set calibration ─────────────────────────────────────────── */
error_code_t voltage_sensor_set_calibration(uint8_t phase,
                                             const voltage_calibration_t *cal)
{
    if (cal == NULL) { return ERR_NULL_POINTER; }
    if (phase > 2) { return ERR_INVALID_PARAMETER; }
    
    memcpy(&s_cal[phase], cal, sizeof(voltage_calibration_t));
    ESP_LOGI(TAG, "Phase %d calibration set: scale=%.3f offset=%ld",
             phase, cal->scale_factor, (long)cal->offset_mv);
    return ERR_OK;
}

/* ── Public: Get calibration ─────────────────────────────────────────── */
error_code_t voltage_sensor_get_calibration(uint8_t phase,
                                             voltage_calibration_t *cal)
{
    if (cal == NULL) { return ERR_NULL_POINTER; }
    if (phase > 2) { return ERR_INVALID_PARAMETER; }
    
    memcpy(cal, &s_cal[phase], sizeof(voltage_calibration_t));
    return ERR_OK;
}

/* ── Public: Calibrate zero ──────────────────────────────────────────── */
error_code_t voltage_sensor_calibrate_zero(uint8_t phase)
{
    if (phase > 2) { return ERR_INVALID_PARAMETER; }
    if (!s_initialized) { return ERR_NOT_INITIALIZED; }
    
    ESP_LOGI(TAG, "Calibrating zero for phase %d...", phase);
    
    /* Take multiple samples and average */
    int32_t sum = 0;
    int count = 0;
    const int SAMPLES = 16;
    
    for (int i = 0; i < SAMPLES; i++) {
        ads_reading_t adc;
        if (ads1115_read_channel(s_ads, phase, &adc) == ERR_OK) {
            sum += adc.millivolts;
            count++;
        }
    }
    
    if (count < SAMPLES / 2) {
        ESP_LOGE(TAG, "Calibration failed: only %d/%d samples", count, SAMPLES);
        return ERR_SENSOR_INVALID;
    }
    
    int32_t avg = sum / count;
    int32_t compensated = (int32_t)(avg * s_cal[phase].divider_ratio);
    s_cal[phase].offset_mv = compensated - ZMPT_CENTER_MV;
    s_cal[phase].is_calibrated = true;
    s_cal[phase].calibration_date = get_time_ms();
    
    ESP_LOGI(TAG, "Phase %d zero calibrated: offset=%ld mV",
             phase, (long)s_cal[phase].offset_mv);
    return ERR_OK;
}

/* ── Public: Is online ───────────────────────────────────────────────── */
bool voltage_sensor_is_online(void)
{
    if (!s_initialized || s_ads == NULL) { return false; }
    return ads1115_is_online(s_ads);
}
